...
async function scheduleOneStream(stream) {
  ...
  try {
    ...
    if (fileNode && fileNode.nodeId) {
      ...
      console.log('Thumbnail file set!');
    }
  } catch (e) {
    console.error('Thumbnail upload notice:', e.message);
  }
...
scheduleOneStream(stream13)
  .then(() => console.log('Successfully completed schedule test for Stream 13!'))
  .catch(console.error);
